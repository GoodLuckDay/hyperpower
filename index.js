  ...
  return class extends React.Component {
    ...
    _drawFrame () {
      ...
      this._particles.forEach((particle) => {
        ...
        this._canvasContext.translate(particle.x,particle.y);
        this._canvasContext.rotate(this._angle * Math.PI/180);
        this._canvasContext.translate(-particle.x,-particle.y);

        this._canvasContext.fillStyle = `rgba(${particle.color.join(',')}, ${particle.alpha})`;
        this._canvasContext.fillRect(Math.round(particle.x - 1), Math.round(particle.y - 1), 10, 10);
